...
fn permissions(network: bool, process: bool, filesystem: &[&str], environment: &[&str], secrets: &[&str]) -> McpPermissions {
    McpPermissions {
        network,
        process,
...
fn blocked_mcp_is_denied() {
    ...
    trust.approve("github", TrustLevel::Blocked, McpPermissions::default(), "1.0.0").unwrap();
    let requested = permissions(false, false, &[], &[], &[]);
...
fn wrong_version_is_denied() {
    let mut trust = TrustStore::default();
    trust.approve("github", TrustLevel::Reviewed, McpPermissions::default(), "1.0.0").unwrap();
...
fn extra_permission_is_denied() {
    let mut trust = TrustStore::default();
    trust.approve("github", TrustLevel::Reviewed, permissions(false, false, &[], &[], &[]), "1.0.0").unwrap();
...
fn approved_mcp_is_allowed() {
    ...
    trust.approve("github", TrustLevel::Reviewed, approved.clone(), "1.0.0").unwrap();
...
fn persistent_trust_round_trip() {
    ...
    store.approve("github", TrustLevel::Reviewed, McpPermissions::default(), "1.0.0").unwrap();
