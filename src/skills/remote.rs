...
impl SkillRegistrySource {
    pub fn parse(value: &str) -> Result<Self> {
        if let Some(repo) = value.strip_prefix("github:") {
            ...
            return Ok(Self::GitHub { repository: repository.to_owned(), reference: reference.to_owned() });
        }
        if let Some(url) = value.strip_prefix("community:") {
            if !(url.starts_with("https://") || url.starts_with("http://")) {
                bail!("community registry must use http(s)");
            }
            return Ok(Self::Community { url: url.trim_end_matches('/').to_owned() });
        }
...
impl RemoteSkillRegistry {
    ...
    pub fn install_github(&self, repository: &str, reference: &str, skill_name: &str, global: &GlobalSkillRegistry) -> Result<Skill> {
        let target = self.cache_dir.join("github").join(repository.replace('/', "__")).join(reference).join(skill_name);
        ...
        if clone_target.exists() { fs::remove_dir_all(&clone_target)?; }
        let status = Command::new("git").args(["clone", "--depth", "1", "--branch", reference, &url, clone_target.to_str().unwrap()]).status()?;
        if !status.success() { bail!("git clone failed for {repository}#{reference}"); }
        ...
        global.get(skill_name)?.context("installed skill could not be parsed")
    }
}
