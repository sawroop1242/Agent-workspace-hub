...
impl RegistryClient {
    ...
    pub async fn fetch_manifest(&self) -> Result<RegistryManifest> {
        ...
        let response = reqwest::get(&url).await.context("failed to contact skill registry")?;
        if !response.status().is_success() {
            bail!("skill registry returned HTTP {}", response.status());
        }
        Ok(response.json::<RegistryManifest>().await.context("invalid registry.json")?)
    }

    pub async fn search(&self, query: &str) -> Result<Vec<RegistrySkill>> {
        let manifest = self.fetch_manifest().await?;
        let query = query.to_ascii_lowercase();
        Ok(manifest.skills.into_iter().filter(|s| {
