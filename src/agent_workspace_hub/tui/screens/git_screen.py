...
class GitScreen(Screen):
    ...
    def on_mount(self) -> None:
        self.query_one(DataTable, id="git-status-table").add_columns("File", "Status")
        self.query_one(DataTable, id="checkpoints-table").add_columns("ID", "Message", "Agent", "Hash", "Date")
        self._load_status()
    ...
    def _load_status(self) -> None:
        t = self.query_one(DataTable, id="git-status-table")
        t.clear()
        try:
    ...
    def _load_checkpoints(self) -> None:
        t = self.query_one(DataTable, id="checkpoints-table")
        t.clear()
        try:
            git = GitEngine(get_settings().workspace_path, self.project_name)
