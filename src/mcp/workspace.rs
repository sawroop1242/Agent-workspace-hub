...
impl WorkspaceMcp {
    ...
    pub fn list_files(&self, relative: &str) -> Result<Vec<WorkspaceFile>> {
        ...
        if !dir.is_dir() { bail!("workspace path is not a directory"); }
        let mut files = Vec::new();
        for entry in fs::read_dir(dir)? {
            let entry = entry?;
            let path = entry.path();
            if path.is_file() {
                files.push(WorkspaceFile { path: path.strip_prefix(&self.root)?.display().to_string(), size: fs::metadata(path)?.len() });
            }
        }
    ...
    pub fn read_file(&self, relative: &str) -> Result<String> {
        ...
        if !path.is_file() { bail!("workspace file not found"); }
        let metadata = fs::metadata(&path)?;
        if metadata.len() > 2 * 1024 * 1024 { bail!("workspace file exceeds 2 MiB limit"); }
    ...
    fn safe_path(&self, relative: &str) -> Result<PathBuf> {
        ...
        let canonical = joined.canonicalize().context("workspace path does not exist")?;
        if !canonical.starts_with(&self.root) { bail!("workspace path escapes project root"); }
        Ok(canonical)
    }
}
