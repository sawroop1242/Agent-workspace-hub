...
impl ComposioProvider {
    pub fn from_env() -> Result<Self> {
        ...
        if api_key.trim().is_empty() { bail!("COMPOSIO_API_KEY is empty"); }
        Ok(Self::new(api_key, std::env::var("COMPOSIO_CONNECTED_ACCOUNT_ID").ok(), std::env::var("COMPOSIO_TOOLKIT").ok()))
    }

    pub fn new(api_key: String, connected_account_id: Option<String>, toolkit: Option<String>) -> Self {
    ...
    fn list_tools(&self) -> Result<Vec<ToolDescriptor>> {
        ...
        Ok(items.into_iter().filter_map(|item| {
            ...
            Some(ToolDescriptor {
                ...
                input_schema: item.get("input_parameters").or_else(|| item.get("input_schema")).cloned().unwrap_or_else(|| json!({"type":"object","properties":{}})),
            })
        }).collect())
    }
